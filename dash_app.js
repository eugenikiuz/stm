...
const NOTES=[
  ...
  "Важно: в августе Яндекс снял с публикации РК на лечение во сне из-за повторной модерации (такое иногда бывает). 2 дня реклама не откручивалась и после перезапуска ушла на переобучение, что дало некоторое ухудшение показателей в моменте и сейчас мы ещё не вернулись к показателям до повторной модерации. Работы по оптимизации продолжаются.",
];
